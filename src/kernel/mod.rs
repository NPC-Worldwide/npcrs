...
The user can see tool outputs directly. Do not re-write or repeat them in your chat response — just reference the relevant parts.",
        ...
        for iteration in 0..max_iterations {
            ...
            let response: crate::r#gen::LlmResponse = crate::r#gen::get_genai_response(
                &provider,
                &model,
                &messages,
                tools.as_deref(),
    ...
    pub async fn run_tool(
        ...
        let (_, executors) = active_npc.resolve_tools(&self.jinxes);
        self.execute_tool(name, args, &executors, active_npc).await
    }
