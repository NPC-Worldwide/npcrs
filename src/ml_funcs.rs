...
pub async fn score_model(model_path: &str, data_json: &str, target: &str) -> Result<f64> {
    ...
    Err(NpcError::Other(
        "ml_funcs.score_model requires Python sklearn runtime. Use npcpy.".into(),
    ))
}
...
pub async fn ensemble_predict(_data_json: &str, _model_paths: &[&str]) -> Result<String> {
    Err(NpcError::Other(
        "ensemble_predict requires Python sklearn runtime. Use npcpy.".into(),
    ))
}
...
pub async fn cross_validate(
    ...
    Err(NpcError::Other(
        "cross_validate requires Python sklearn runtime. Use npcpy.".into(),
    ))
}
