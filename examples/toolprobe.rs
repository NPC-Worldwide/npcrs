...
fn main() {
    ...
    let mut tools: Vec<_> = team
        .jinxes
        .values()
        .filter_map(|j| j.to_tool_def())
        .collect();
    ...
    // Turn 1
    let resp =
        npcrs::r#gen::get_llamacpp_response(&model, &messages, Some(&tools), 512, 0.7, 4096, -1)
            .expect("inference");
    let mut msg = resp.message.clone();
    println!("TURN1 content: {:?}", msg.content);
    println!(
        "TURN1 tool_calls: {:?}",
        msg.tool_calls.as_ref().map(|t| t
            .iter()
    ...
    if let Some(calls) = msg.tool_calls.take() {
        ...
        let resp2 =
            npcrs::r#gen::get_llamacpp_response(&model, &hist, Some(&tools), 512, 0.7, 4096, -1)
                .expect("inference2");
        let m2 = resp2.message.clone();
        println!("TURN2 content: {:?}", m2.content);
        println!(
            "TURN2 tool_calls: {:?}",
            m2.tool_calls.as_ref().map(|t| t
                .iter()
                .map(|c| c.function.name.clone())
                .collect::<Vec<_>>())
        );
